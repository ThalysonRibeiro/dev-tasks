...
// First, create a mock version of the entire page module
...
// Typed mocks with any to avoid strict type checking in tests
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
// Create a real Dashboard implementation for testing
  ...
  return async function TestDashboard() {
    ...
    const totalItens = (group: Array<Group & { item: Item[] }>): number => {
      ...
      for (const element of group) {
        element.item.forEach((item: Item) => {
          count.push(item);
        });
      }
      return count.length;
...
describe('Dashboard Page', () => {
  ...
  describe('Desktop Item Count Logic', () => {
    it('should calculate correct item counts', async () => {
      ...
      const group2 = createMockGroup('group-2', 'Group 2', []); // grupo sem tarefas
