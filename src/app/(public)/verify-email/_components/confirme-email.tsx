  ...
  useEffect(() => {
    const verifyEmail = async () => {
      ...
      try {
        ...
        if (res.ok) {
          setMessage(data.message || 'E-mail confirmado com sucesso! Você já pode fazer login na sua conta.');
          setIsSuccess(true);
        } else {
        ...
        {isSuccess ? (
          <div className="space-y-4">
            ...
            <div className="bg-zinc-800 border rounded-lg p-4">
              <p className="text-sm">
                Redirecionando para a página de login em <span className="font-bold">{countdown}</span> segundos...
              </p>
            </div>
