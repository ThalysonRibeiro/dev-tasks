...
export default function FolderAnimation({ children }: { children: React.ReactNode }) {
  const titlesPageFolder = [
    { title: "1_____________", class: "z-[1] group-hover:-top-8 group-hover:rotate-6 group-hover:left-[8%]" },
    { title: "2_____________", class: "z-[2] delay-75 group-hover:-top-5 group-hover:left-[12%] group-hover:rotate-4" },
    { title: "3_____________", class: "z-[3] delay-150 group-hover:-top-2 group-hover:left-[14%] group-hover:rotate-2" },
  ];
  return (
    <div className="group relative w-full h-full" style={{ perspective: '1000px' }}>
      ...
      {titlesPageFolder.map((titlePges, index) => (
        <div
          key={index}
          className={cn("w-[85%] h-[85%] bg-white rounded-lg border border-gray-300",
